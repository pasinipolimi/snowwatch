    ...
    $.warpingMatcher.prototype = {
        ...
        */
        ...
        generateCoordinates: function  () {
			...
			var centerAzimuth = (parseFloat(startImageX + endImageX) / 2 - parseFloat(plugin.$renderPhotoContainer.position().left)) / parseFloat(plugin.$render.width()) * 360;
			centerAzimuth = goodMod(centerAzimuth, 360);
			
			
			var centerPitch = ( (parseFloat(plugin.$renderPhotoContainer.position().top) + parseFloat(plugin.$render.height()) / 2) - (parseFloat(startImageY + endImageY) / 2) ) / parseFloat(plugin.$render.width()) * 360;
			centerPitch = goodMod(centerPitch, 360);
			if (centerPitch > 180) {
				centerPitch = centerPitch - 360;
			}
			
			var photoRealXScale = plugin.$photo[0].naturalWidth / plugin.$canvas.width();
            var photoRealYScale = plugin.$photo[0].naturalHeight / plugin.$canvas.height();
			

			var pts = _(plugin.warper.oriPoints).map(function (d,j) {
                    if(d.hiddenPoint == false)
                        return {renderPeakId: plugin.warper.dstPoints[j].matchingPeak, x: parseInt(d.x * photoRealXScale), y:parseInt(d.y * photoRealYScale)};
                }).filter(function (d,i) {
                        return d != undefined;
                });

            /** addAlignment?mediaId=TST1&centerAzimuth=100&centerPitch=20&centerRoll=0&reportedBy=TSTUSER1&alignmentPeaks=[**/
            var imageId = plugin.options.photoId;

            var fov= plugin.options.photoScaleFactor * 360 * plugin.options.hSizePhoto / plugin.options.hSizeRender;
            
            $.ajax({ 
                type: 'get',
                url: engineHost+'addAlignment?centerAzimuth='+centerAzimuth+"&centerPitch="+centerPitch+"&centerRoll=0&mediaId="+imageId+"&reportedBy=SWP2&alignmentPeaks="+JSON.stringify(pts)+"&hFov="+fov, 
                dataType: 'jsonp',
                //data: {centerAzimuth: centerAzimuth, centerPitch: centerPitch, centerRoll: 0, mediaId: imageId, reportedBy: "SWP2", alignmentPeaks: JSON.stringify(pts)},
			...
			/*
			//USE FOR EACH PEAK COORD ON PHOTO ESTIMATION (EVEN NOT MANUALLY ALIGNED)
        ...
        */
        _init: function(options) {
            ...
            plugin.$increaseZoomButton = $('.increase-zoom'); //plugin.$el.find('.increase-zoom');
            ...
            _(plugin.options.peaks).each(function  (d,i) {
                //debugger;
                //htmlPeaks += "<div class='render-peak' title='"+ d.auxName +" (" + d.auxGpsAlt + " m)' data-peakid='"+d.peakId+"' style='left:"+ d.x +"px;top:"+ d.y +"px'></div>";
                htmlPeaks += "<div class='render-peak' title='"+ d.auxName +" (" + d.auxGpsAlt + " m)' data-peakid='"+ d.id +"' style='left:"+ d.x +"px;top:"+ d.y +"px'></div>";
            });
            ...
            $("#sliderZoom").on({
                set: function(){
                  plugin._changeFov(parseFloat($("#sliderZoom").val()));
                }
            });
            ...
            plugin.$increaseZoomButton.click(function (e) {
				...
				*/
				var oldPhotoScaleFactor = plugin.options.photoScaleFactor;
				plugin.options.photoScaleFactor = plugin.options.photoScaleFactor / 1.1;
                plugin._resizeChildren();
				
				//plugin.$renderPhotoContainer.css("left", (oldLeft - plugin.$renderPhotoContainer.width() * (1 - oldPhotoScaleFactor / plugin.options.photoScaleFactor) / 2)  + "px");
				var startImageX = plugin.$canvasContainer.position().left;
				var endImageX = startImageX + plugin.$photo.width();
				
				var startImageY = plugin.$canvasContainer.position().top;
				var endImageY = startImageY + plugin.$photo.height();				
				plugin.$renderPhotoContainer.css("left", (oldLeft - (parseFloat(startImageX + endImageX) / 2 - parseFloat(plugin.$renderPhotoContainer.position().left)) * (oldPhotoScaleFactor / plugin.options.photoScaleFactor - 1)) + "px");
				plugin.$renderPhotoContainer.css("top", (oldTop - ( (parseFloat(plugin.$renderPhotoContainer.position().top) + parseFloat(plugin.$render.height()) / 2) - (parseFloat(startImageY + endImageY) / 2) ) * (oldPhotoScaleFactor / plugin.options.photoScaleFactor - 1)) + "px");				
            ...
            plugin.$decreaseZoomButton.click(function (e) {
				...
				plugin.currentZoom = plugin.currentZoom - plugin.options.zoomUnit;
				*/
				plugin.options.photoScaleFactor = plugin.options.photoScaleFactor + 0.1;
                plugin._resizeChildren();
                //plugin._renderCanvas();
            });
        ...
        _changeFov: function(fov){
            ...
            var oldPhotoScaleFactor = plugin.options.photoScaleFactor;
            //plugin.options.photoScaleFactor = plugin.options.photoScaleFactor / 1.1;
            plugin.options.photoScaleFactor = fov * plugin.options.hSizeRender / 360 / plugin.options.hSizePhoto;
            plugin._resizeChildren();
